...
#          f.write "#{row['guid']},#{row['brand']},#{row['end_date'].strftime("%F %T")},#{row['source']},#{row['product']},#{row['trace_id']},#{row['start_date'].strftime("%F %T")}\n"
        else
          f.write "#{row['guid']},#{row['brand']},#{row['end_date'].strftime("%F %T")},#{row['source']},#{row['product']},#{row['trace_id']},#{row['start_date'].strftime("%F %T")}\n"
        end
        nCounter += 1
...
task :cassandra_test do
  ...
  sTableName = Cfg.config['cassandraCluster']['keyspace'] + '.' + Cfg.config['tables']['entitlements']
  nFound = 0
  cql = "SELECT * FROM #{sTableName} WHERE start_date>='#{start_date_min}' AND start_date<='#{start_date_max}' ALLOW FILTERING"
